...
void test(std::string name, std::list<Eigen::Vector3d> waypoints, double maxDeviation, const Eigen::Vector3d& maxVelocities, const Eigen::Vector3d& maxAccelerations, double timeStep) {
...
void test1() {
	Eigen::Vector3d waypoint(4);
	std::list<Eigen::Vector3d> waypoints;
	...
	Eigen::Vector3d maxVelocities(4);
	maxVelocities << 1.3, 0.67, 0.67, 0.5;
	Eigen::Vector3d maxAccelerations(4);
...
Eigen::Vector3d sampleConfig() {
	Eigen::Vector3d maxPositions(7);
	maxPositions << 115.0, 105.0, 90.0, 70.0, 130.0, 90.0, 55.0;
	maxPositions *= 3.14159 / 180.0;
	Eigen::Vector3d config(7);
...
void randomTests() {
	...
	Eigen::Vector3d maxVelocities(7);
	maxVelocities << 1.309, 1.6406, 3.2812, 2.618, 5.236, 3.4907, 3.4907;
	Eigen::Vector3d maxAccelerations = maxVelocities;
